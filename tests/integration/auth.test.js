...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      const res = await request(app).post('/v1/auth/refresh-tokens').set('Cookie', `refreshToken=${refreshToken}`).expect(httpStatus.OK);
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      ...
      await request(app).post('/v1/auth/refresh-tokens').set('Cookie', `refreshToken=${refreshToken}`).expect(httpStatus.UNAUTHORIZED);
    });
...
describe('Auth middleware', () => {
  test('should call next with no errors if access token is valid', async () => {
    await insertUsers([userOne]);
    const req = httpMocks.createRequest({ cookies: { accessToken: userOneAccessToken } });
    const next = jest.fn();
  ...
  test('should call next with no errors if user does not have required rights but userId is in params', async () => {
    await insertUsers([userOne]);
    const req = httpMocks.createRequest({
      cookies: { accessToken: userOneAccessToken },
